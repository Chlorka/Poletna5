	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			else
			{
				...
				$sAction = "ADD";
                if($arParams["IBLOCK_ID"] == $_REQUEST["iblock_id"]) {
                    if($arParams["ONLY"] == $_REQUEST["only"]) {
        ...
        if($arParams["IBLOCK_ID"] == $_REQUEST["iblock_id"]) {
            ...
            if (empty($arResult["ERRORS"])) {
                ...
                $sRedirectUrl .= urlencode($arParams["USER_MESSAGE_" . $sAction])."&iblock_id=".$arParams["IBLOCK_ID"];
                $arResult["MESSAGE_ADD"] = 'Y';
