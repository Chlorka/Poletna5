...
<form class="form_send"
	...
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
		...
		<div>
            ...
            <?if($propertyID != 'PREVIEW_TEXT'):?>
                    ...
                    ?>
                    <?/*if($INPUT_TYPE != 'T'):?>
                        <label><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?>(ќЅя«ј“≈Ћ№Ќќ)<?endif?></label>
                    <?endif;*/?>
                    <?
                    switch ($INPUT_TYPE):
							case "USER_TYPE":
    ...
    * - пол€, об€зательные дл€ заполнени€
    ...
    <div class="ww-grid">
        ...
        <input type="submit" name="iblock_submit" value="«аказать" />
...
<?/*?><p class="order-text">ћы перезвоним вам в течении часа рабочего времени и согласуем заказ</p><?*/?>
</form>
</div>
