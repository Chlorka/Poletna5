...
/*if ($USER->IsAdmin()) {
    echo "<pre>"; print_r($arSection["~NAME"]); echo "</pre>";
} 
*/
		}
		$this->EndResultCache();
...
foreach($arResult["SECTIONS"] as $arSection)
{
	...
	$arSelect = array("ID", "IBLOCK_ID", "DETAIL_PAGE_URL", "IBLOCK_SECTION_ID", "NAME" , "PREVIEW_PICTURE");
	$arFilter = array(
		"ACTIVE" => "Y",
		"IBLOCK_ID" => $arParams["IBLOCK_ID"],
	...
	while($arElement = $rsElements->GetNext())
	{
		...
		$Element_list[$arSection["ID"]][]=$arElement;
	}
	...
	$aMenuLinksNew[$menuIndex++] = array(
		...
		array(
			...
			"DEPTH_LEVEL" => $arSection["DEPTH_LEVEL"],
			"ID"=> $arSection["ID"],
			"ELEMENT_LIST"=> $Element_list[$arSection["ID"]],
		),
	);
